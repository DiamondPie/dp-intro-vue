...
- Components: `Editor/EditorBar.vue` (floating status/save bar, mounted in `index.vue`), `Editor/EditorWorks.vue` (editable card grid that `Works.vue` swaps in), `Editor/EditorInput.vue` (block field with EN/ZH/href/img tag). Both mount points use `<ClientOnly>` + `LazyEditor*` and `useEditor().enabled` (always `false` on the server), so nothing editor-related reaches the prerendered HTML and the editor code lives in its own chunks. Copy is under `editor.*` in the locale files.
- Reordering uses `sortablejs` via `@vueuse/integrations` `useSortable` (pinned to 14.3.0 to match `@vueuse/nuxt`). Pass it a **writable** computed (getter through `siteContent`, setter assigning `siteContent.value.works`), never the read-only one from `useSiteContent()`: given a ref it copies the array and assigns `list.value`, which a read-only computed drops silently (DOM moves, data doesn't).
- Only `works` is editable so far; photos / friends / commits follow the same pattern (see `temp/content-manager-plan.md` phase C).
